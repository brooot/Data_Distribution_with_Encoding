    ...
    while need_to_forwardrecv.value:
        ...
        if r_list:
            ...
            if addr in Neigh_ADDR:
                ...
                with lock_of_L:
                    # 解码
                    print("收到转发层数据, 已解码: %3d/%3d" % (len(L_decoded), piece_num) )
...
def comm_with_peer(ADDR, L_decoded, L_undecoded, source_not_confirmed, lock_of_L, need_to_forwardrecv):
    ...
    while source_not_confirmed.value:
        ...
        forward_data = get_forward_data_1_degree(L_decoded, L_undecoded)

        print(time.ctime().split(" ")[4], "转发层 发送 码字 ---> ", dest_addr)

        send_with_loss_prob(sockfd_withPeer, forward_data, dest_addr)
...
def comm_with_source(ADDR, L_decoded, L_undecoded, source_not_confirmed, lock_of_L, need_to_forwardrecv):
    ...
    while True:
        ...
        print("从源收到 ", list(m_info_set)[0])

        need_info_set -= m_info_set

        print("需要的码字: ", need_info_set)
        ...
        if len(L_decoded) == piece_num:
            ...
            # 将解码数据记录到excel表格中
            with open("Decoding_Log" + str(ADDR) + ".csv","w",newline="", encoding='utf-8') as datacsv:
